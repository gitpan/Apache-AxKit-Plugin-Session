...
BEGIN {
	...
	$Apache::AxKit::Plugin::Session::VERSION = 0.90;
}
...
#-------------
    ...
    my $session = $r->notes('SESSION_ID') || $r->pnotes('COOKIES')->{$auth_type.'_'.$auth_name} || '';
	...
	if ($session =~ m/^\E$prefix\Q/) {
		...
		if (substr($rest,0,1) eq '/') {
			$r->status(REDIRECT);
			$self->external_redirect("/$session$rest");
			return REDIRECT;
		}
	} else {
		undef $session;
...
#------------------------
    ...
    my $uri;
    # this is a redirect. sort out details later
    $uri = Apache::URI->parse($r, $r->header_out('Location') || $r->prev->header_out('Location') || $r->pnotes('INPUT')->{'url'});
    my $same_host = (!$uri->hostname || lc($uri->hostname) eq lc($r->hostname));
...
EOF
    	...
    	if ($session && $r->main->parsed_uri->path =~ /^$session/) {
...
sub handler {
	my ($r) = @_;
	my $self = __PACKAGE__;
	$r->auth_type($self) unless $r->auth_type;
...
Session management only: (minimal configuration, no URL session ID tracking)

    AxAddPlugin Apache::AxKit::Plugin::Session

Minimal config with authentication/authorization (protect site below /protected):
...
Full featured configuration: (again, example protection below /protected)
    ...
    # Which session module to use: (supported are File, DB_File and Counted, File is default)
    ...
    # Note: This must be a valid session ID; you may need to create
    # the session record/file/whatever manually
    ...
    ### Directory settings (this may go into .htaccess as well)
    #
    <Location /protected>
        AuthType Apache::AxKit::Plugin::Session
        AuthName AxKitSession
...
adding hyperlinks to your page, change http://www.foo.com to /redirect?http://www.foo.com
...
=item * set cookie on changed SID; without this, Apache::Session::Counted will not work
	very well with cookies (i.e. not at all)
